...
def add_user(telegram_id, phone, user_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT OR REPLACE INTO users VALUES (?, ?, ?)", (telegram_id, phone, user_id)
    )
    conn.commit()
    conn.close()
...
def get_user_by_telegram_id(telegram_id: int) -> dict | None:
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute("SELECT telegram_id, phone, user_id FROM users WHERE telegram_id=?", (telegram_id,))
    row = cur.fetchone()
    conn.close()
    if not row:
        return None
    return dict(zip(["telegram_id", "phone", "user_id"], row))
